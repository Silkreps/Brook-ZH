...
export const FORBIDDEN_PROCUREMENT = /consult(?:ing|ancy|ant)?|supervision|advisory|technical assistance|project management|implementation support|research|study|training|audit|goods only|equipment only|awarded|contract(?:ed| signed)?|signed|cancelled|canceled|closed|expired|completed|limited to national|domestic bidders only|national bidders only|仅限本国|监理|咨询|顾问|研究|培训|技术援助|项目管理|实施支持|已授标|已签约|已成交|已关闭|已取消|已过期/i;
