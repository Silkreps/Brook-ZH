...
const FORBIDDEN = /consult(ing|ancy)|supervision|advisory|study|audit|goods only|equipment only|awarded|cancelled|canceled|closed|limited to national|domestic bidders only|national bidders only|仅限本国|监理|咨询|已授标|已取消/i;
...
export function isEngineeringProcurement(project: ProjectCandidate) {
  ...
  return ALLOWED.test(text) && !FORBIDDEN.test(text);
}
export function decideProjectGate(project: ProjectCandidate): GateDecision {
  const text = `${project.procurementType ?? ""} ${project.stage ?? ""} ${project.title ?? ""} ${project.noticeText ?? ""}`;
  if (project.awarded || project.cancelled || project.officialLinkValid === false || project.authentic === false || FORBIDDEN.test(text)) return "禁止推送";
  ...
  if (!project.amountUsd || project.amountUsd < amountThresholdFor(project)) return "禁止推送";
