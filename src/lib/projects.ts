...
export function isPublicProject(p: TenderProject) {
  ...
  return hasPublishedChineseTitle(p)
...
export function filterAndSortProjects(projects: TenderProject[], filters: ProjectFilters) {
  let rows = projects.filter((p) => {
    ...
    if (filters.pending && !(p.reviewStatus === "pending" || p.status === "待人工核实")) return false;
...
export function displayStatus(p: TenderProject) { if (p.reviewStatus === "pending" || p.status === "待人工核实") return "待人工核实"; if (p.deadlineAt && new Date(p.deadlineAt).getTime() - Date.now() <= 7 * DAY) return "即将截止"; return "正在招标"; }
