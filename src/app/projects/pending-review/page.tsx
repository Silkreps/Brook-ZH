...
export default async function PendingReviewPage({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  ...
  return <main className="shell">
    ...
    <div className="project-list">{result.rows.length === 0 ? <div className="card empty">暂无待人工核实项目</div> : result.rows.map((project) => <article className="card project-card" key={project.id}>
      ...
      <dl className="project-facts"><div><dt>国家</dt><dd>{project.country}</dd></div><div><dt>项目编号</dt><dd>{project.procurementNo ?? "官方未公布"}</dd></div><div><dt>原始名称</dt><dd>{project.titleEn}</dd></div><div><dt>发布日期</dt><dd>{project.publishedAt ? new Date(project.publishedAt).toLocaleDateString("zh-CN") : "官方未公布"}</dd></div><div><dt>截止日期</dt><dd>{project.deadlineAt ? new Date(project.deadlineAt).toLocaleDateString("zh-CN") : "官方未公布"}</dd></div><div><dt>官方来源</dt><dd>{project.financier}</dd></div><div><dt>官方链接</dt><dd>{project.links.filter((link) => link.isOfficial).map((link) => <a key={link.url} href={link.url} target="_blank" rel="noreferrer">{link.label}</a>)}</dd></div></dl>
